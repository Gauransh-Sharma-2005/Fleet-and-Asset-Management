  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-12 space-y-8">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h2 className="text-2xl font-black text-white tracking-tight">FleetStream - Black Box File System Manager</h2>
      ...
      {statusMsg && (
        <div className="bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 px-4 py-3 rounded-xl text-sm flex items-center gap-2 font-mono">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        {/* Form Section */}
        <form onSubmit={handleSaveLog} className="bg-slate-900/80 backdrop-blur-md p-6 rounded-2xl border border-indigo-500/15 space-y-4">
          ...
          <button 
            type="submit" 
            className="w-full bg-indigo-600 hover:bg-indigo-500 text-white font-bold rounded-xl px-4 py-3 text-sm flex items-center justify-center gap-2 shadow-lg shadow-indigo-600/20 transition-all mt-2"
          >
            <HardDrive className="w-4 h-4" /> Save Log to Server FS (`fs.appendFile`)
          </button>
        </form>

        {/* Output / Terminal Section */}
        <div className="bg-slate-900/80 backdrop-blur-md p-6 rounded-2xl border border-indigo-500/15 flex flex-col space-y-4">
          <div className="flex items-center justify-between">
            ...
            <button 
              ...
              className="px-3 py-1.5 bg-emerald-600/20 hover:bg-emerald-600/30 text-emerald-400 border border-emerald-500/30 rounded-lg text-xs font-mono flex items-center gap-1.5 transition-colors"
            >
              <RefreshCw className={`w-3.5 h-3.5 ${loading ? 'animate-spin' : ''}`} /> Fetch Logs (`fs.readFile`)
            </button>
          </div>

          <div className="flex-1 bg-slate-950 border border-slate-800 rounded-xl p-4 font-mono text-xs text-emerald-400/90 overflow-y-auto h-64 whitespace-pre-wrap">
